...
def main():
    ...
    parser.add_argument('--variant', choices=['difference', 'no_difference', 'baseline'], default='difference')
    ...
    from methods import EffB1_ZoomNeXt_FSPResidual
    assert torch.cuda.is_available(), 'Requested CUDA device unavailable'
    config_name = {'difference': 'icod_train_fsp_residual.py', 'no_difference': 'icod_train_fsp_residual_nodiff.py', 'baseline': 'icod_train_fsp_residual_baseline.py'}[args.variant]
    cfg = Config.fromfile('configs/' + config_name)
    assert cfg.model_name == 'EffB1_ZoomNeXt_FSPResidual'
    ...
    assert model.use_fsp_residual == (args.variant != 'baseline')
    assert model.use_difference == (args.variant != 'no_difference')
    assert model.fusion_stages == (3, 2)
    assert not any(hasattr(model, n) for n in ('fde_1', 'fde_2', 'edge_head', 'sg_1', 'sg_2'))
    print('MODEL:', type(model).__name__, 'VARIANT:', args.variant, 'residual:', model.use_fsp_residual, 'difference:', model.use_difference, 'stages:', model.fusion_stages, 'guidance=False; frequency=False; edge=False; MSCA=False', flush=True)
    ...
    subprocess.run([sys.executable, 'check_fsp_residual.py'], check=True)
    runs = root / 'fsp_residual_runs'
    ...
    for name in ('main_for_image.py', 'requirements.txt', 'run_fsp_residual.py', 'check_fsp_residual.py'):
    ...
    (root / 'last_fsp_residual_run.txt').write_text(str(run), encoding='utf-8')
    ...
    cmd = [sys.executable, '-u', 'main_for_image.py', '--config', 'selected_config.py',
           ...
           '--output-dir', str(run / 'results'), '--info', 'fsp_residual_' + args.variant]
    if cfg.pretrained:
