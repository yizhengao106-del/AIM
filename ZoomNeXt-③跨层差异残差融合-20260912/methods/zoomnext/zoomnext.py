...
from .layers import EdgeReconstructionHead, FrequencyDetailEnhancer, MHSIU, RGPU, SimpleASPP
...
class EffB1_ZoomNeXt_FDER(EffB1_ZoomNeXt):
    def __init__(self, pretrained, num_frames=1, input_norm=True, mid_dim=64, siu_groups=4, hmu_groups=6, **kwargs):
        super().__init__(
        ...
        self.edge_head = EdgeReconstructionHead(mid_dim)
        self.edge_loss_weight = 0.3

    def set_loss_config(self, loss_cfg=None):
        super().set_loss_config(loss_cfg=loss_cfg)
        self.edge_loss_weight = self.loss_cfg.get("edge_loss_weight", 0.3)
    ...
    def body(self, data):
        logits, _ = self.body_with_edge(data=data)
        return logits

    def forward(self, data, iter_percentage=1, **kwargs):
        logits, edge_logits = self.body_with_edge(data=data)
        ...
        if self.training:
            ...
            edge_target = self.mask_to_edge(mask)
            edge_loss = F.binary_cross_entropy_with_logits(edge_logits, edge_target, reduction="mean")
            losses.append(self.edge_loss_weight * edge_loss)
            loss_str.append(f"edge_{self.edge_loss_weight:.2f}: {edge_loss.item():.5f}")

            return dict(
                vis=dict(sal=prob, edge=edge_logits.sigmoid()),
                loss=sum(losses),
