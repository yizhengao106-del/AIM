...
class FrequencyDetailEnhancer(nn.Module):
    def __init__(self, in_dim):
        super().__init__()
        self.low_proj = ConvBNReLU(in_dim, in_dim, 1)
        self.high_proj = ConvBNReLU(in_dim, in_dim, 3, 1, 1)
        self.gate = nn.Sequential(
            nn.Conv2d(in_dim, in_dim, 1, bias=False),
            nn.Sigmoid(),
        )
        self.fuse = ConvBNReLU(2 * in_dim, in_dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        high = high * self.gate(high)
        return self.fuse(torch.cat([low, high], dim=1)) + x
