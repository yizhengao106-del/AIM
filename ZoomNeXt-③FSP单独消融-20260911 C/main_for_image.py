...
def parse_cfg():
    ...
    for key, value in vars(args).items():
        ...
        if isinstance(value, bool):
            ...
            cli_overrides[key] = value
    cfg.merge_from_dict(cli_overrides)
...
def main():
    ...
    model = model_class(
        num_frames=1, pretrained=cfg.pretrained, use_checkpoint=cfg.use_checkpoint,
        **cfg.get("model_cfg", {}),
    )
    model.set_loss_config(cfg.get("loss_cfg", None))
    LOGGER.info(model_code)
    model.to(cfg.device)

    load_from = cfg.get("load_from", None)
    if load_from:
        io.load_weight(model=model, load_path=load_from, strict=True)

    LOGGER.info(f"Number of Parameters: {sum((v.numel() for v in model.parameters(recurse=True)))}")
    evaluate = cfg.get("evaluate", False)
    has_test = cfg.get("has_test", False)

    if not evaluate:
        train(model=model, cfg=cfg)

    if evaluate or has_test:
        io.save_weight(model=model, save_path=cfg.path.final_state_net)
